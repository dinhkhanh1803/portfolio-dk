...
function drawGame(
  ...
  for (const drop of state.drops) {
    const color = drop.type === "wide" ? "#54d9d3" : drop.type === "slow" ? "#ffbb58" : "#f37866";
    context.save();
    ...
    context.fillText(drop.type === "wide" ? "W" : drop.type === "slow" ? "S" : "×3", drop.x, drop.y + 1);
    context.restore();
  }
}
...
export default function NeonBreakerGame() {
  ...
  const copy = useMemo(() => language === "vi" ? {
    ...
    intro: "Phá sạch 5 màn gạch neon, giữ combo và săn power-up trước khi mất cả 3 mạng.",
    ...
    intro: "Smash through five handcrafted neon boards, hold your combo, and catch power-ups before three lives run out.",
  ...
  useEffect(() => {
    ...
    const next = {
      ...
      unlockedLevel: Math.max(progress.unlockedLevel, Math.min(5, view.phase === "level-clear" ? view.level + 1 : view.level)),
    };
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    pointerX(event);
    unlockAudio();
    if (stateRef.current.phase === "ready") launch();
  };
    ...
    <main className={styles.page} data-theme={theme} data-reduced-motion={reducedMotion}>
      ...
      <section className={styles.gameShell}>
        <div className={styles.hud}>
          ...
          <div><span>{copy.level}</span><strong>{view.level}/5</strong></div>
